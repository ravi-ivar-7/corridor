...
class SettingsDialog(Gtk.Window):
    def __init__(self):
        Gtk.Window.__init__(self, title="Corridor Settings")
        ...
        self.ws_entry.set_text(self.config.get("websocket_url", ""))
        ...
        self.http_entry.set_text(self.config.get("http_url", ""))
...
"""
    ...
    def on_save_clicked(self, button):
        ...
        if not self.setup_autostart(enable_autostart):
            self.show_toast("⚠ Settings saved, but autostart setup failed")
            return
        ...
        dialog.format_secondary_text(
            ...
            "Corridor needs to restart for changes to take effect.\n"
        ...
        if response == Gtk.ResponseType.YES:
            ...
            # Kill only the corridor binary (not python dialogs)
            # Use pgrep to find the exact corridor process
            try:
                ...
                if pgrep_result.stdout.strip():
                    # Kill the corridor process
                    subprocess.run(["pkill", "-x", "corridor"])
                    time.sleep(1.5)
            except:
                # Fallback to pkill
                subprocess.run(["pkill", "-x", "corridor"])
            ...
            # Start new corridor instance with --autostart flag
            subprocess.Popen([corridor_path, "--autostart"],
